...
fn parse_csv_accounts(text: &str) -> Result<Vec<ProfileDraft>, String> {
...
/// 把草稿序列化成 CSV（含表头）。
fn build_csv_accounts(drafts: &[ProfileDraft]) -> Result<String, String> {
...
fn unique_profile_name(name: &str, taken: &HashSet<String>) -> String {
...
fn import_profiles_impl<R: Runtime>(
    app: &AppHandle<R>,
    drafts: Vec<ProfileDraft>,
    conflict: String,
    dry_run: bool,
    ...
    for (offset, draft) in drafts.into_iter().enumerate() {
        ...
        if let Err(message) = sanitize_profile_name(&draft.name) {
            errors.push(ImportRowError { row, name: draft.name.clone(), message });
            continue;
        }
            ...
            match conflict.as_str() {
                ...
                "rename" => {
                    ...
                    match build_profile(app, draft, profiles.len() + created.len()) {
                        ...
                        Err(message) => errors.push(ImportRowError { row, name: requested, message }),
                    }
                }
                _ => {
                    ...
                    match build_profile(app, draft, order) {
                        Ok(mut incoming) => {
                            if let Some(existing) = profiles.iter_mut().find(|p| p.name == requested) {
    ...
    Ok(ImportReport { created, updated, skipped, errors })
}
...
#[tauri::command]
async fn pick_export_file(app: AppHandle, default_name: String) -> Result<Option<String>, String> {
    use tauri_plugin_dialog::DialogExt;
    let picked = app
        .dialog()
        .file()
...
#[tauri::command]
fn parse_accounts_csv(text: String) -> Result<Vec<ProfileDraft>, String> {
    parse_csv_accounts(&text)
}

/// 草稿 -> CSV 文本（含表头）。
#[tauri::command]
fn build_accounts_csv(drafts: Vec<ProfileDraft>) -> Result<String, String> {
...
mod tests {
    ...
    fn csv_parse_skips_header_and_blank_lines() {
        let text = "name,default_url,url_mode\n账号A,https://a.com/,inherit\n\n   \n账号B,,custom\n";
    ...
    fn import_reports_bad_rows_and_imports_the_rest() {
        ...
        let drafts = vec![
            ProfileDraft { name: "好账号".to_string(), ..Default::default() },
            // 空昵称应被逐行记错，而不是让整批失败。
            ProfileDraft { name: "   ".to_string(), ..Default::default() },
            // 非法网址同样只影响这一行。
            ProfileDraft {
                name: "坏网址".to_string(),
    ...
    fn import_conflict_strategies_behave_as_documented() {
        ...
        let report = import_profiles_impl(
            &handle,
            vec![ProfileDraft { name: "同名".to_string(), ..Default::default() }],
            "rename".to_string(),
            false,
        )
        .unwrap();
        ...
        assert!(load_profiles(&handle).unwrap().iter().any(|p| p.name == "同名"));
    ...
    fn import_dry_run_does_not_persist() {
        ...
        let report = import_profiles_impl(
            &handle,
            vec![ProfileDraft { name: "试试".to_string(), ..Default::default() }],
            "rename".to_string(),
            true,
        )
        .unwrap();
    ...
    fn import_rejects_unknown_conflict_strategy() {
        ...
        let result = import_profiles_impl(
            &handle,
            vec![ProfileDraft { name: "x".to_string(), ..Default::default() }],
            "bogus".to_string(),
            false,
        );
